...
async function showTab(tabName) {
    ...
    if (tabName === 'invoices') {
        ...
        if(addBtn) addBtn.classList.add('hidden'); // Hide + button on invoice screen
        renderInvoices();
    } else {
        daily.classList.remove('hidden');
        invoices.classList.add('hidden');
        if(addBtn) addBtn.classList.remove('hidden'); // Show + button for daily
        renderList();
    }
    ...
    customers.forEach(cust => {
        ...
        card.innerHTML = `
            <div>
                ...
                <p class="text-[10px] text-gray-400 font-bold uppercase">${saladCount} Delivered • ${addonCount} Add-ons</p>
            </div>
            <div class="text-right">
                <p class="text-xl font-black text-green-700 leading-none">₹${Math.round(total)}</p>
...
async function generateCustomerInvoice(custId, monthYear) {
    ...
    // HEADER
    try { doc.addImage('logo.jpg', 'JPEG', 15, 15, 30, 30); } catch(e) {}
    
    doc.setTextColor(40, 40, 40);
    doc.setFontSize(10);
    doc.text("Grabb a green salads & grills", 50, 25);
    doc.text("9346379970, 9121448100", 50, 30);
